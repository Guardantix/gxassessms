...
- External tools -> raw artifact storage
  Data: JSON, HTML, and Markdown result files plus serialized `RawToolOutput` manifests.
  Channel: filesystem writes into operator-chosen output directories, then into engagement artifact directories via `ArtifactManager.save_raw_outputs`.
  Guarantees: path traversal checks are applied when GxAssessMS writes its own engagement artifacts in `src/gxassessms/persistence/artifacts.py::_validate_path_within_root`.
  Validation: adapter-specific raw-output validators check expected JSON shape before parsing, but no cryptographic integrity binding exists between collected results and later replay.
- Filesystem artifacts -> replay boundary
  Data: `raw-output/*.json` manifests and referenced result-file paths.
  Channel: local file reads in `src/gxassessms/pipeline/replay.py::load_raw_outputs`.
  Guarantees: `RawToolOutput.model_validate_json()` enforces schema shape.
  Validation: adapters re-validate parsed structures, but replay does not constrain referenced file paths to stay under the engagement directory.
...
| Replay manifests and referenced files | `mseco replay` or `run_from(PARSE)` | Filesystem artifacts -> parser | Replay trusts manifest file paths more than the engagement root. | `src/gxassessms/pipeline/replay.py::load_raw_outputs`, `src/gxassessms/pipeline/_runner.py::_rehydrate_upstream_state` |
...
3. Attacker goal: falsify a report or read unintended JSON content. Steps: modify `raw-output/*.json` replay manifests to point at another file or another engagement's results -> operator runs `mseco replay` or `consolidate --reparse` -> adapter validation reads the attacker-selected path -> wrong findings are persisted or cross-engagement data appears in a report.
...
| TM-003 | Local user or process tampering with persisted replay manifests or referenced result files | Attacker can modify `raw-output/*.json` or files referenced by `RawToolOutput.file_manifest` before replay. | Replay loads the manifest, then adapter validation reads attacker-selected file paths without constraining them to the engagement root. | False findings, cross-engagement contamination, and limited arbitrary JSON file disclosure under the analyst account. | Report integrity, engagement isolation, customer data confidentiality | Raw manifests must satisfy the `RawToolOutput` schema, and adapters perform structural JSON validation before parsing. Evidence: `src/gxassessms/pipeline/replay.py::load_raw_outputs`, `src/gxassessms/adapters/scubagear/adapter.py::_validate_and_load_results`, `src/gxassessms/adapters/maester/adapter.py::_validate_and_load_tests`. | Replay does not reject absolute or out-of-root paths; there is no signature or hash binding between collection-time artifacts and replay-time inputs. | Store only relative paths under the engagement directory, reject out-of-root paths during replay, and record content hashes or signatures for raw artifacts at collection time. | Log every replay path read; alert on absolute paths, `..` segments, or references outside the engagement directory. | Medium | High | high |
...
| `src/gxassessms/pipeline/replay.py` | Replay loader is the strongest artifact-integrity boundary and currently lacks path confinement for referenced files. | TM-003, TM-005 |
| `src/gxassessms/persistence/artifacts.py` | Governs artifact storage, archive/restore/purge, and the only explicit path-traversal checks in the repo. | TM-003, TM-004, TM-006 |
